...
class SVM():
    ...
    def simpleSMO(self, dataMatIn, classLabels, C, toler, maxIter):
        ...
        alphas = np.mat(np.zeros((m, 1)))  # [100 rows x 1 column's 0]
        iter = 0
        ...
        while (iter < maxIter):
            ...
            for i in range(m):
                ...
                if ((labelMat[i] * Ei < -toler) and (alphas[i] < C)) or ((labelMat[i] * Ei > toler) and (alphas[i] > 0)):
                    ...
                    eta = dataMatrix[i, :] * dataMatrix[i, :].T + dataMatrix[j, :] * \
                        dataMatrix[j, :].T - 2.0 * dataMatrix[i, :] * \
                        dataMatrix[j, :].T  # 7.106
                    ...
                    alphas[i] += labelMat[j] * labelMat[i] * \
                        (alphaJold - alphas[j])  # 7.109

                    # the update is in the oppostie direction
                    b1 = b - Ei - labelMat[i] * (alphas[i] - alphaIold) * dataMatrix[i, :] * dataMatrix[i, :].T \
                        - labelMat[j] * (alphas[j] - alphaJold) * \
                        dataMatrix[i, :] * dataMatrix[j, :].T  # 7.115
                    b2 = b - Ej - labelMat[i] * (alphas[i] - alphaIold) * dataMatrix[i, :] * dataMatrix[j, :].T \
                        - labelMat[j] * (alphas[j] - alphaJold) * \
                        dataMatrix[j, :] * dataMatrix[j, :].T  # 7.116
